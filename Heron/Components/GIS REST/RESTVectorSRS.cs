...
namespace Heron
{
    public class RESTVectorSRS : HeronComponent
    {
        //Class Constructor
        public RESTVectorSRS() : base("Get REST Vector", "RESTVector", "Get vector data from ArcGIS REST Services", "GIS REST")
        {
                ...
                if (run)
                {
                    ...
                    for (int iLayer = 0; iLayer < dataSource.GetLayerCount(); iLayer++)
                    {
                        ...
                        while ((feat = ogrLayer.GetNextFeature()) != null)
                        {
                            ...
                            ///Assign geometry to heronSRS if not set in source
                            if (geomUser.GetSpatialReference() == null) { geomUser.AssignSpatialReference(heronSRS); }
